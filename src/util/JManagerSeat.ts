...
class SeatManager {
    ...
    unitySeatWithSeatData(type, seatData) {
        ...
        if (type === 'ytb') {
            ...
            for (let seatKey in seats) {
                if (seats.hasOwnProperty(seatKey)) {
                    const seat = seats[seatKey];
                    seat.rowId = seat.rowId || '1';
                    seat.columnId = seat.columnId || '1';
                }
            }
        }
        // 淘票票预处理
        ...
        if (type === 'maoyan' || type === 'meituan' || type === 'dazhong') {
            maxRow = seatData.maxRow ? seatData.maxRow : 0;
            maxColumn = seatData.maxColumn ? seatData.maxColumn : 0;
    ...
    handleTaoBaoSeatData(seatData) {
        ...
        for (let key in seatMap) {
            if (seatMap.hasOwnProperty(key)) {
                ...
                let rowId: string = seatMap[key].rowId;
                let columnId: string = seatMap[key].columnId;
    ...
    smartSeatsWithSeats(type, seatList) {
        ...
        if (type === 'taobao') {
            ...
            smartSeats = this.smartSeatsWithAllSeats(seatList);
        }
     ...
     * @returns {Array}
     */
    smartSeatsWithAllSeats(seatList) {
    ...
    smartSeatsWithTBSeats(seatList) {
        return seatList.map((seatModel) => {
            let row = Number.parseInt(seatModel.rowNo);
            let col = Number.parseInt(seatModel.columnNo);
            //后台说直接用，不让转换202004271759
            let rowOriNumber: string = seatModel.rowId;
            let colOriNumber: string = seatModel.columnId;
            return {
                rowOriNumber,
                colOriNumber,
                row,
                col,
                rowNumber: rowOriNumber,
                colNumber: colOriNumber,
                seatModel
            };
        }).map(bridgeModel => {
            let seatRowModel = bridgeModel.seatModel;
